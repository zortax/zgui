...
fn distribute_remaining_free_space(flex_lines: &mut [FlexLine], constants: &AlgoConstants) {
    for line in flex_lines {
        ...
        // zgui local patch (see /vendor/README.md): once auto margins have absorbed the free
        // space, justify-content must be resolved against zero remaining space — the spec's step
        // 12 hands the space to the margins *instead of* to alignment. Upstream fell through with
        // `free_space` unchanged, so any `justify-content` other than flex-start moved every item
        // by the absorbed space a second time, pushing the later ones past the container's edge.
...
fn perform_absolute_layout_on_absolute_children(
    ...
    for order in 0..tree.child_count(node) {
        ...
        let fixed = child_style.is_fixed_position();
        let aspect_ratio = child_style.aspect_ratio();
        ...
        #[cfg(feature = "content_size")]
        {
            ...
            // A fixed-position child is anchored to the viewport rather than to any scrollable
            // ancestor: it cannot be scrolled to, so it adds no scrollable overflow here.
            if !fixed && size_content_size_contribution.has_non_zero_area() {
