...
pub trait CoreStyle {
    ...
    /// Whether the node is fixed-position: laid out exactly like [`Position::Absolute`], but
    /// contributing nothing to its containing block's content size. A fixed box is anchored to
    /// the viewport rather than to any scrollable ancestor, so it cannot be scrolled to and adds
    /// no scrollable overflow — a viewport-sized overlay inside a scroll container must not give
    /// that container a scrollbar.
    #[inline(always)]
