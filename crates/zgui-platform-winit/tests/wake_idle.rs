...
//! load and a machine at rest give different answers to the same question. So it is asserted as
//! counts instead — frames run, deadline arrivals reported, turns of the loop taken — every one of
//! which a spinning loop inflates by orders of magnitude and a parked one leaves at rest.
//!
//! The quiet is measured between two signals rather than from the start, because opening a window
//! is not nothing: the platform maps it, tells the application its size, and asks for a first
//! paint. What has to be zero is what happens *after* all that, with nobody touching anything.
...
fn main() {
    ...
    // turns in that time; one that blocks takes the handful the platform had something to say on.
    let turns = app.turns();
    println!("the idle loop took {turns} turns over {QUIET:?} of quiet");
    assert!(
        turns < 64,
