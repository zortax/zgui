//! The document every budget is measured against.
//!
//! The gallery of the `styled` example: twelve panels, real text in several sizes and weights, and
//! a row of swatches one of which is picked by a signal. The text is what makes it the right
//! fixture — a document with three words in it cannot tell a glyph cache that works from one that
//! does not.
