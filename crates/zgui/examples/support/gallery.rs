//! The shipped `styled` gallery, as a view a measuring harness can build.
//!
//! The component, the style sheet and the size are the example's own, so that what is measured is
//! the application a person runs rather than a fixture written to be easy to measure.

#![allow(dead_code)]

use zgui::prelude::*;

/// The width the gallery opens at, in CSS pixels.
pub(crate) const WIDTH: f32 = 1080.0;

/// The height the gallery opens at, in CSS pixels.
pub(crate) const HEIGHT: f32 = 720.0;

/// The gallery, as a view.
