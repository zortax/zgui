//! The styled gallery, opened under an application identifier taken from the environment.
//!
//! It is `examples/styled.rs` with one change: the application identifier and the window title come
//! from `ZGUI_DIAG_APPID`, so a compositor rule can address this window and no other. Everything
//! that is measured — the view, the style sheet, the initial size — is the shipped example's.
//!
//! Run it with `ZGUI_DIAG_APPID=some-id cargo run --release -p zgui --example styled_diag`.
...
fn main() -> Result<(), zgui::Error> {
    let id = std::env::var("ZGUI_DIAG_APPID").unwrap_or_else(|_| "zgui-diag-styled".to_owned());
    app()
