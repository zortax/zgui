...
//! keys. Even the one-pair case needs a table this module does not hold: `keyboard.h` names the 27
//! accents a `KT_DEAD` entry indexes and says nothing about what each composes into.
//!
//! **Key codes stop at 255.** A console keymap holds `NR_KEYS` entries, and `NR_KEYS` is 256. A key
//! the kernel numbers above that has no entry at all, and [`Console::entry`] refuses it.
//!
//! # Key codes
//!
...
//! # The keyboard mode
//!
//! A console keyboard is in one of the modes `kd.h` names, and [`Console::mode`] reports which. In
//! every mode except [`Mode::Unicode`] the kernel answers `K_HOLE` for each entry above the
//! eight-bit types, so a German keymap read in [`Mode::Translate`] keeps its umlauts, which are
//! Latin-1, and loses its euro sign. In [`Mode::Raw`] and [`Mode::Off`] the console delivers
//! nothing from the keymap at all, and what is read is the layout that *would* apply.
//!
//! # State
//!
//! Nothing here follows a key over time. A caller that wants the character a key press produces
//! holds its own modifier state and asks for the map that state selects.
...
/// `NR_KEYS` past that field would fail this build rather than leave [`index`] refusing codes the
/// keymap has.
const _: () = assert!(
    sys::NR_KEYS == 256,
...
impl Modifiers {
    ...
    ///
    /// Every value is one. A keymap defines the maps it wants, and a map it left out reads as
    /// holes throughout.
...
/// `x & 0xff`. Each variant is one of the `KT_*` types and carries the value as the kernel wrote
/// it.
///
...
pub enum Entry {
    ...
    Latin(u8),
    /// A character of that set that caps lock also acts on. `KT_LETTER`.
    Letter(u8),
    ...
    ///
    /// Two cases read as one. The keymap may bind nothing here, and the kernel also answers a hole
    /// for an entry it declines to report in the mode the console is in. [`Console::mode`] says
    /// whether that second case is possible: in [`Mode::Unicode`] a hole is the keymap's own, and
    /// in every other mode a hole may be either.
    Hole,
    /// The map itself was never loaded. `K_NOSUCHMAP`, which `KDGKBENT` answers for key code zero
    /// alone, so asking for that code is how a caller learns whether a combination has a map.
    NoSuchMap,
    ...
    Pad(u8),
    /// A dead key, as an index into the kernel's own list of accents. `KT_DEAD`.
    Dead(u8),
    ...
    Cursor(u8),
    /// A modifier key, as the bit it holds down. `KT_SHIFT`.
    Modifier(u8),
    /// One digit of a code point typed on the keypad. `KT_ASCII`.
    Ascii(u8),
    /// A modifier lock. `KT_LOCK`.
    Lock(u8),
    /// A modifier lock that holds for one key. `KT_SLOCK`.
    StickyLock(u8),
...
impl Entry {
    ...
    pub const fn decode(value: u16) -> Self {
        // The two entries `keyboard.h` composes by name. A hole is the commonest value in any
        // keymap, because every key a map leaves unbound is one.
        ...
        match (value >> 8) as u32 {
            ...
            // Past `KT_BRL` the kernel stops packing a type, and the whole value is a code point
            // exclusive-ored with `0xf000`. `dumpkeys` reads it the same way. A code point from
            // `0xf000` to `0xfeff` lands back among the types and is lost, which is the encoding's
            // own limit rather than this decoding's.
            _ => Self::Unicode(value ^ 0xf000),
        }
    }
    ...
    /// character after the next key. A keypad key produces one under num lock, through a table no
    /// header carries. A function key produces a string, held where `KDGKBSENT` reads it. A meta
    /// entry produces its character with an escape byte before it or with the high bit set, which
    /// is a decision about the terminal rather than about the layout.
...
impl Console {
    ...
    /// What the keymap holds for `key` under `modifiers`.
    ///
    /// # Errors
    ///
    /// Returns [`Error::Unusable`] when `key` is past the last code a console keymap has, and
    /// [`Error::Ioctl`] when the kernel refuses.
    pub fn entry(&self, key: Key, modifiers: Modifiers) -> Result<Entry> {
        ...
        Ok(Entry::decode(request.kb_value))
    }
}

/// The console's index for `key`.
///
/// # Errors
///
/// Returns [`Error::Unusable`] for a code past the last entry a console keymap has. Truncating one
/// would name another key: `KEY_FN_F1` is `0x1d2`, and the low byte of that is `KEY_PRINT`.
fn index(key: Key) -> Result<u8> {
    u8::try_from(key.raw()).map_err(|_| {
        Error::Unusable(format!(
            "a console keymap has {} entries, and key code {} is past the last of them",
            sys::NR_KEYS,
            key.raw()
        ))
    })
...
mod tests {
    ...
    /// so the packing is written here. The test below holds it against the two the header composes
    ...
    #[test]
    fn the_two_answers_for_an_absent_entry_have_names_of_their_own() {
        // A hole is every unbound key in every map, so a caller meets it constantly.
        // `K_NOSUCHMAP` comes back for key code zero alone and says the whole map was never
        // loaded, which is how a caller asks whether a modifier combination exists at all.
        assert_eq!(Entry::decode(sys::ZGUI_K_HOLE as u16), Entry::Hole);
        assert_eq!(
            Entry::decode(sys::ZGUI_K_NOSUCHMAP as u16),
            Entry::NoSuchMap
        );
        // Both are `KT_SPEC`, and the rest of that type stays an action the console takes.
        assert_eq!(Entry::decode(pack(sys::KT_SPEC, 7)), Entry::Special(7));
    }

    #[test]
    fn an_entry_past_the_last_type_is_a_code_point() {
        // Above `KT_BRL` the kernel stops packing a type, and the whole value is a code point
        // exclusive-ored with `0xf000`. The euro sign is the case that matters: it is on a German
        // layout, and no eight-bit type can hold it.
        assert_eq!(Entry::decode(0x20ac ^ 0xf000), Entry::Unicode(0x20ac));
        assert_eq!(Entry::decode(0x20ac ^ 0xf000).character(), Some('€'));
        // A code point below the eight-bit boundary is written the same way and reads back whole.
        assert_eq!(Entry::decode(0x0041 ^ 0xf000), Entry::Unicode(0x41));
    }
    ...
    fn a_modifier_combination_is_the_index_of_the_map_it_selects() {
        ...
        assert!((Modifiers::SHIFT | Modifiers::ALTGR).contains(Modifiers::ALTGR));
        assert!(!Modifiers::SHIFT.contains(Modifiers::ALTGR));
    }

    #[test]
    fn a_key_code_past_the_last_entry_of_a_console_keymap_is_refused() {
        ...
        // print key does.
        assert_eq!(index(Key::KEY_A).expect("KEY_A is 30"), 30);
        assert_eq!(index(Key::new(255)).expect("255 is the last index"), 255);
        assert!(index(Key::new(256)).is_err());
        assert!(index(Key::new(0x1d2)).is_err(), "`KEY_FN_F1` has no entry");
    }
    ...
    #[test]
    fn only_a_unicode_console_reports_an_entry_above_the_eight_bit_types() {
